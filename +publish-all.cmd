...
if not defined APP_VER set "APP_VER=(?????놁쓬)"
echo !BD!?꾩옱 踰꾩쟾: !GR!!APP_VER!!RS!
echo !DG!Directory.Build.props 湲곗? ??諛고룷 ??踰꾩쟾???뺤씤?섏꽭??!RS!
echo.
set /p "CONFIRM=怨꾩냽 諛고룷?섏떆寃좎뒿?덇퉴? [Y/N] "
if /i not "!CONFIRM!"=="Y" (
    echo !RE!諛고룷媛 痍⑥냼?섏뿀?듬땲??!RS!
    exit /b 0
)
echo.
...
if !FAIL! equ 0 (
    set "MSG=Publish-All ?깃났: !PASS!/!TOTAL! ?꾨줈?앺듃 諛고룷 ?꾨즺"
    set "LV=Info"
) else (
    set "MSG=Publish-All ?ㅽ뙣: !PASS!/!TOTAL! ?깃났, ?ㅽ뙣:!FAILED!"
    set "LV=Error"
)
