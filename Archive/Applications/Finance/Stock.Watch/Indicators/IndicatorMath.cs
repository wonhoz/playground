...
public static class IndicatorMath
{
    ...
    public static double[] Rsi(IReadOnlyList<double> closes, int period = 14)
    {
        ...
        for (int i = 1; i < closes.Count; i++)
        {
            ...
            if (i == period)
            {
                // i == period-1 에서 시드 완료, 여기서부터 Wilder 평활
            }
    ...
    /// <summary>MACD(기본 12/26/9). 반환: MACD선, 시그널선, 히스토그램.</summary>
    public static (double[] Macd, double[] Signal, double[] Histogram) Macd(
        IReadOnlyList<double> closes, int fast = 12, int slow = 26, int signal = 9)
    {
        ...
        // 시그널은 MACD의 EMA. NaN 구간을 건너뛰고 계산.
        var valid = macd.Where(v => !double.IsNaN(v)).ToList();
        var sigValid = Ema(valid, signal);
        var sig = new double[closes.Count];
        var hist = new double[closes.Count];
        int vi = 0;
        for (int i = 0; i < closes.Count; i++)
        {
            if (double.IsNaN(macd[i])) { sig[i] = hist[i] = double.NaN; continue; }
            sig[i] = sigValid[vi];
            hist[i] = double.IsNaN(sig[i]) ? double.NaN : macd[i] - sig[i];
            vi++;
        }
