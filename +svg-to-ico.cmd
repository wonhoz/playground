...
echo.
echo %C1%  ════════════════════════════════════════════════════%C0%
echo %C2%     SVG  %DIM%→%C0%  ICO Converter  %DIM%(16 / 32 / 48 / 256 px)%C0%
echo %C1%  ════════════════════════════════════════════════════%C0%
echo.
...
:input_svg
set "SVG_PATH="
set /p "SVG_PATH=%C2%  SVG 파일 경로를 입력하세요%C0%  %DIM%(드래그 앤 드롭 가능)%C0%%C2%: %C0%"
...
:: ── dotnet run 실행 ──────────────────────────────────────────
dotnet run --project "%TMP_DIR%\Converter.csproj" --configuration Release -- "%SVG_PATH%" "%ICO_PATH%" 2>&1
