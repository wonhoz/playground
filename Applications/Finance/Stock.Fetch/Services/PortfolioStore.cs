...
/// <para>저장 경로는 <see cref="AppConfig.PortfolioPath"/>(비어 있으면 문서\StockFetch\portfolio.json).
...
public sealed class PortfolioStore
{
    ...
    /// <summary>설정의 경로(빈 값이면 문서 폴더 기본 경로).</summary>
    public static string ResolvePath(AppConfig cfg) =>
        string.IsNullOrWhiteSpace(cfg.PortfolioPath) ? DefaultPath : cfg.PortfolioPath;
