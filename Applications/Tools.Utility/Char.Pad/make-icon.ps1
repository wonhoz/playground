...
public static class IconGen {
    public static byte[] MakePng(int size) {
        ...
        SolidBrush bg = new SolidBrush(Color.FromArgb(255, 75, 25, 150));
        g.FillPath(bg, path);
        bg.Dispose();

        Pen pen = new Pen(Color.FromArgb(180, 160, 100, 255), Math.Max(1f, size / 32f));
        g.DrawPath(pen, path);
        pen.Dispose();

        int fs = (int)(size * 0.48);
        Font font  = new Font("Segoe UI Symbol", fs, FontStyle.Regular, GraphicsUnit.Pixel);
        SolidBrush tb = new SolidBrush(Color.FromArgb(255, 230, 195, 255));
        StringFormat sf = new StringFormat();
        sf.Alignment     = StringAlignment.Center;
        sf.LineAlignment = StringAlignment.Center;
        g.DrawString("\u2328", font, tb, new RectangleF(0, 0, size, size), sf);
