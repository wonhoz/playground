...
public static class IconGen {
    public static byte[] MakePng(int size) {
        ...
        SolidBrush bg = new SolidBrush(Color.FromArgb(255, 15, 55, 30));
        g.FillPath(bg, path);
        bg.Dispose();

        using (var pen = new Pen(Color.FromArgb(180, 0, 192, 112), Math.Max(1f, size/32f)))
            g.DrawPath(pen, path);

        int fs = (int)(size * 0.44);
        Font font = new Font("Consolas", fs, FontStyle.Bold, GraphicsUnit.Pixel);
        SolidBrush tb = new SolidBrush(Color.FromArgb(255, 0, 220, 120));
        StringFormat sf = new StringFormat();
        sf.Alignment = StringAlignment.Center;
        sf.LineAlignment = StringAlignment.Center;
        g.DrawString("Cc", font, tb, new RectangleF(0, 0, size, size), sf);
