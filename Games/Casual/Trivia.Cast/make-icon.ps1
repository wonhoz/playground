...
public static class IconGen {
    public static byte[] MakePng(int size) {
        ...
        using (var bg = new SolidBrush(Color.FromArgb(255, 26, 21, 40)))
            g.FillPath(bg, path);

        using (var pen = new Pen(Color.FromArgb(200, 168, 85, 247), Math.Max(1f, size/32f)))
            g.DrawPath(pen, path);

        // Question mark symbol
        int fs = (int)(size * 0.52);
        using (var font = new Font("Segoe UI", fs, FontStyle.Bold, GraphicsUnit.Pixel))
        using (var tb = new SolidBrush(Color.FromArgb(255, 255, 215, 0)))
        {
            StringFormat sf = new StringFormat();
            sf.Alignment = StringAlignment.Center;
            sf.LineAlignment = StringAlignment.Center;
            g.DrawString("?", font, tb, new RectangleF(0, -(float)(size*0.04), size, size), sf);
        }

        // Small star decorations
        int starSize = (int)(size * 0.15);
        using (var starBrush = new SolidBrush(Color.FromArgb(200, 168, 85, 247)))
        {
            g.FillEllipse(starBrush, size - starSize - (int)(size*0.08), (int)(size*0.08), starSize, starSize);
        }
